...
-- 全局变量
do
    BG.FBtable = BG.FBtable or {}
    BG.FBtable2 = BG.FBtable2 or {}
    BG.FBIDtable = BG.FBIDtable or {}
    BG.lootQuality = BG.lootQuality or {}
    BG.difficultyTable = BG.difficultyTable or {}
    BG.diffIDTbl = BG.diffIDTbl or {}
    BG.phaseFBtable = BG.phaseFBtable or {}
    BG.bossPositionStartEnd = BG.bossPositionStartEnd or {}
    BG.FBfromBossPosition = BG.FBfromBossPosition or {}
    BG.Movetable = BG.Movetable or {}
    BG.options = BG.options or {}
    BG.itemCaches = BG.itemCaches or {}
    ...
    BG.spFB = {}
    BG.fakuanIsFirst = {}
    -- 默认暴雪模板；若 BGLite 已加载则保持 BiaoGe 模板
    BG.editTemplate = _G.BiaoGe_InputBoxTemplate and "BiaoGe_InputBoxTemplate" or "InputBoxTemplate"
    BG.editSearchTemplate = _G.BiaoGe_SearchBoxTemplate and "BiaoGe_SearchBoxTemplate" or "SearchBoxTemplate"
    BG.scrollTemplate = "UIPanelScrollFrameTemplate"

    function BG.ResolveUITemplates()
        if _G.BiaoGe_InputBoxTemplate then
            BG.editTemplate = "BiaoGe_InputBoxTemplate"
            BG.editSearchTemplate = _G.BiaoGe_SearchBoxTemplate and "BiaoGe_SearchBoxTemplate" or "SearchBoxTemplate"
        end
        if _G.BiaoGe_ModernScrollFrameTemplate then
            BG.scrollTemplate = "BiaoGe_ModernScrollFrameTemplate"
        elseif _G.TCO_ModernScrollFrameTemplate then
            BG.scrollTemplate = "TCO_ModernScrollFrameTemplate"
        else
            BG.scrollTemplate = "UIPanelScrollFrameTemplate"
        end
    end

    BG.Init2(BG.ResolveUITemplates)
    BG.notLootBossIDs = {}
...
do
    ...
    -- 表格基础信息
    do
        local function FBAlreadyRegistered(FB)
            for _, existing in ipairs(BG.FBtable) do
                if existing == FB then
                    return true
                end
            end
        ...
        local function AddDB(FB, instanceID, phase, maxplayers, lootQuality,
            ...
            if not FBAlreadyRegistered(FB) then
    ...
    -- 表格（BGLite/BiaoGe 已初始化时跳过，避免破坏其 UI 与函数）
    do
        if BG.UpdateZhiChuPercent then
            -- 仅合并心愿 UI 结构，供 TCO 心愿单使用
    ...
    do
        ...
        local Interface = ns.MediaRoot .. "sound\\"
        BG.soundAuthor = {
...
BG.Init(function()
    ...
    -- BGLite 共存：拍卖聊天记录等（Migrate 已合并，此处兜底）
    ...
    -- BGLite 共存：tab 按钮依赖 options.alpha（BGLite/Options.lua 在别名切换前可能写在另一张表）
    if ns.EnsureBGLiteCoexistDefaults then
        ns.EnsureBGLiteCoexistDefaults()
    end
    ...
    do
        ...
        function TCO_InputBoxTemplate_OnLoad(self)
