...
do
    ...
    BG.Init(function()
        ...
        BG.FilterClassItemDB = BG.FilterClassItemDB or { ShuXing_filter = {}, MainAttribute_filter = {} }
    end)
...
do
    ...
    function BG.SetListzhuangbei(self)
        if not self or not self.FB or not self.bossnum then return end
        ...
        local diffName = BG.diffIDTbl[FB] and BG.diffIDTbl[FB][GetRaidDifficultyID()]
        if self.hopenandu then
            diffName = BG.difficultyTable[FB] and BG.difficultyTable[FB][self.hopenandu]
        end
        if not diffName then diffName = "N" end

        local maxBossRow = self.hopenandu and HopeMaxb and HopeMaxb[FB] or (Maxb[FB] and (Maxb[FB] - 1))
        if maxBossRow and bossnum > maxBossRow then return end

        local loots = BG.Loot[FB] and BG.Loot[FB][diffName] and BG.Loot[FB][diffName]["boss" .. bossnum]
        local isHope = self.hopenandu

        local uiParent = (BG.WishlistWindow and BG.WishlistWindow:IsVisible() and BG.WishlistWindow.content)
            or BG.WishlistWindow
            or UIParent

        -- 背景框
        local f = CreateFrame("Frame", nil, uiParent, "BackdropTemplate")
        if uiParent == UIParent then
            f:SetFrameStrata("TOOLTIP")
            f:SetFrameLevel(500)
        else
            f:SetFrameLevel(uiParent:GetFrameLevel() + 50)
        end
        f:SetBackdrop({
            bgFile = "Interface/ChatFrame/ChatFrameBackground",
            edgeFile = "Interface/Tooltips/UI-Tooltip-Border",
        ...
        -- 内容层置于 backdrop 之上，避免底色盖住列表项且拦截点击
        local listRoot = CreateFrame("Frame", nil, f)
        listRoot:SetPoint("TOPLEFT", 4, -4)
        listRoot:SetPoint("BOTTOMRIGHT", -4, 4)
        listRoot:SetFrameLevel(f:GetFrameLevel() + 100)
        listRoot:EnableMouse(false)
        f.listRoot = listRoot
        ...
        text:SetJustifyH("LEFT")
        if self.hopenandu then
            text:SetText(BG.STC_w1(L["（点击选择装备，SHIFT+点击发送装备，CTRL+点击查看该部位的其他可选装备）"]))
        else
        ...
        for t = 1, 2 do
            for i = 1, MaxI do
                ...
                local bt = CreateFrame("EditBox", nil, listRoot, BG.editTemplate)
                bt:SetSize(btwidth, btheight)
                bt:SetFrameLevel(listRoot:GetFrameLevel() + 10)
                bt:SetTextInsets(14, 0, 0, 0)
                bt:SetAutoFocus(false)
                bt:EnableMouse(true)
                bt:SetScript("OnEditFocusGained", function(box)
                    box:ClearFocus()
                end)
                if BG.TCOPatchHopeEditBg then
                ...
                if t == 1 and i == 1 then
                    bt:SetPoint("TOPLEFT", listRoot, "TOPLEFT", 6, -1)
                    frameright = bt
                ...
                if bt.itemID then
                    ...
                    local function LoadButtonText()
                        ...
                        if BG.OnItemLoad then
                            local handle = BG.OnItemLoad(itemID)
                            if handle and handle.ContinueOnItemLoad then
                                handle:ContinueOnItemLoad(function()
                                    SetButtonText(bt)
                                end)
                                return
                            end
                    ...
                    local function PickItem(self, button)
                    ...
                    bt:SetScript("OnMouseDown", PickItem)
                end
            end
        end
        f:SetSize(btwidth * 2 + 10 + 10, (btheight + 2) * MaxI + 5 + 3)
        text:SetWidth(f:GetWidth())
        if f:GetBottom() and f:GetBottom() < 20 then
            f:ClearAllPoints()
            f:SetPoint("BOTTOMLEFT", self, "TOPLEFT", -9, -2)
        end
    ...
    -- 保存 TCO 版实现，防止 BGLite 后加载覆盖导致弹层挂到隐藏的 MainFrame
    BG.TCOSetListzhuangbei = BG.SetListzhuangbei
    function BG.OpenHopeLootPicker(owner)
        if not owner then return false end
        local fn = BG.TCOSetListzhuangbei or BG.SetListzhuangbei
        if not fn then
            print("|cff00BFFF<TCO>|r 装备列表函数未加载")
            return false
        end
    ...
    function BG.HookScrollBarShowOrHide(scroll, alwaysHide)
        if scroll.ScrollBar.ThumbButton then
            ...
            if scroll.ScrollBar:GetOrientation() == "HORIZONTAL" then
                TCO_ModernHorizontalScrollFrameTemplate_Update(scroll)
            else
                TCO_ModernScrollFrameTemplate_Update(scroll)
            end
            return
        end
...
do
    ...
    function TCO_ModernScrollThumbButton_UpdateShape(self)
    ...
    function TCO_ModernScrollThumbButton_UpdatePosition(bar)
    ...
    function TCO_ModernScrollFrameTemplate_Update(self)
        ...
        if bar.ThumbButton then
            ...
            TCO_ModernScrollThumbButton_UpdateShape(bar.ThumbButton)
            TCO_ModernScrollThumbButton_UpdatePosition(bar)
        end
    ...
    function TCO_ModernScrollFrameTemplate_SetScrollExtent(self, visibleExtent, totalExtent)
        self.modernVisibleExtent = visibleExtent
        self.modernTotalExtent = totalExtent
        TCO_ModernScrollFrameTemplate_Update(self)
    end

    function TCO_ModernScrollFrameTemplate_ClearScrollExtent(self)
        self.modernVisibleExtent = nil
        self.modernTotalExtent = nil
        TCO_ModernScrollFrameTemplate_Update(self)
    end

    function TCO_ModernScrollFrameTemplate_OnLoad(self)
        self:EnableMouse(true)
        self:EnableMouseWheel(true)
        self.SetScrollExtent = TCO_ModernScrollFrameTemplate_SetScrollExtent
        self.ClearScrollExtent = TCO_ModernScrollFrameTemplate_ClearScrollExtent
        self.ScrollBar.scrollStep = BG.scrollStep
        TCO_ModernScrollFrameTemplate_Update(self)
    end

    function TCO_ModernScrollFrameTemplate_OnScrollRangeChanged(self, xRange, yRange)
    ...
    function TCO_ModernScrollFrameTemplate_OnVerticalScroll(self, offset)
    ...
    function TCO_ModernScrollFrameTemplate_OnMouseWheel(self, delta)
        local bar = self.ScrollBar
        local minValue, maxValue = bar:GetMinMaxValues()
    ...
    function TCO_ModernScrollBarTemplate_OnLoad(self)
    ...
    function TCO_ModernScrollBarTemplate_OnValueChanged(self, value)
        local scroll = self:GetParent()
        if TCO_ModernScrollThumbButton_UpdatePosition then
            TCO_ModernScrollThumbButton_UpdatePosition(self)
        end
        if not scroll or scroll.modernVisibleExtent ~= nil then return end
    ...
    function TCO_ModernScrollBarTemplate_OnMouseWheel(self, delta)
        TCO_ModernScrollFrameTemplate_OnMouseWheel(self:GetParent(), delta)
    end

    function TCO_ModernScrollBarTemplate_OnEnter(self)
        -- self:GetThumbTexture():SetColorTexture(unpack(c2))
    end

    function TCO_ModernScrollBarTemplate_OnLeave(self)
        -- self:GetThumbTexture():SetColorTexture(unpack(c1))
    end

    function TCO_ModernScrollTrackButton_OnLoad(self)
        self:SetFrameLevel(self:GetParent():GetFrameLevel() + 1)
        self:EnableMouseWheel(true)
    end

    function TCO_ModernScrollTrackButton_OnMouseDown(self, button)
    ...
    function TCO_ModernScrollTrackButton_OnMouseWheel(self, delta)
        TCO_ModernScrollBarTemplate_OnMouseWheel(self:GetParent(), delta)
    end
    ...
    function TCO_ModernScrollThumbButton_OnLoad(self)
        local bar = self:GetParent()
        ...
        TCO_ModernScrollThumbButton_UpdateShape(self)
        TCO_ModernScrollThumbButton_UpdatePosition(bar)
        SetModernScrollThumbButtonColor(self, c1)
    end

    function TCO_ModernScrollThumbButton_OnEnter(self)
        if not self.modernDragging then
            SetModernScrollThumbButtonColor(self, c2)
        end
    end

    function TCO_ModernScrollThumbButton_OnLeave(self)
        if not self.modernDragging then
            SetModernScrollThumbButtonColor(self, c1)
        end
    end

    function TCO_ModernScrollThumbButton_OnMouseDown(self, button)
        if button ~= "LeftButton" then return end
    ...
    function TCO_ModernScrollThumbButton_OnMouseUp(self, button)
    ...
    function TCO_ModernHorizontalScrollThumbButton_UpdateShape(self)
    ...
    function TCO_ModernHorizontalScrollThumbButton_UpdatePosition(bar)
        local thumbButton = bar.ThumbButton
        if not thumbButton then return end
    ...
    function TCO_ModernHorizontalScrollFrameTemplate_Update(self)
        local bar = self.ScrollBar
        if not bar then return end
        ...
        if bar.ThumbButton then
            ...
            TCO_ModernHorizontalScrollThumbButton_UpdateShape(bar.ThumbButton)
            TCO_ModernHorizontalScrollThumbButton_UpdatePosition(bar)
        end
    ...
    function TCO_ModernHorizontalScrollFrameTemplate_SetScrollExtent(self, visibleExtent, totalExtent)
        self.modernVisibleExtent = visibleExtent
        self.modernTotalExtent = totalExtent
        TCO_ModernHorizontalScrollFrameTemplate_Update(self)
    end

    function TCO_ModernHorizontalScrollFrameTemplate_ClearScrollExtent(self)
        self.modernVisibleExtent = nil
        self.modernTotalExtent = nil
        TCO_ModernHorizontalScrollFrameTemplate_Update(self)
    end

    function TCO_ModernHorizontalScrollFrameTemplate_OnLoad(self)
        self:EnableMouse(true)
        self:EnableMouseWheel(true)
        self.SetScrollExtent = TCO_ModernHorizontalScrollFrameTemplate_SetScrollExtent
        self.ClearScrollExtent = TCO_ModernHorizontalScrollFrameTemplate_ClearScrollExtent
        self.ScrollBar.scrollStep = BG.scrollStep
        TCO_ModernHorizontalScrollFrameTemplate_Update(self)
    end

    function TCO_ModernHorizontalScrollFrameTemplate_OnScrollRangeChanged(self, xRange, yRange)
        if self.modernVisibleExtent ~= nil then
            TCO_ModernHorizontalScrollFrameTemplate_Update(self)
            return
        end
    ...
    function TCO_ModernHorizontalScrollFrameTemplate_OnHorizontalScroll(self, offset)
        if self.modernVisibleExtent ~= nil then return end
        if self.modernScrollSyncing then return end
        self.modernScrollSyncing = true
        self.ScrollBar:SetValue(offset)
    ...
    function TCO_ModernHorizontalScrollFrameTemplate_OnMouseWheel(self, delta)
        local bar = self.ScrollBar
        local minValue, maxValue = bar:GetMinMaxValues()
        local value = bar:GetValue() - delta * (bar.scrollStep or BG.scrollStep or 20)
        bar:SetValue(math.max(minValue, math.min(value, maxValue)))
    ...
    function TCO_ModernHorizontalScrollBarTemplate_OnLoad(self)
        self:SetMinMaxValues(0, 0)
        self:SetValueStep(1)
        self:EnableMouseWheel(true)
        self:Hide()
    ...
    function TCO_ModernHorizontalScrollBarTemplate_OnValueChanged(self, value)
        local scroll = self:GetParent()
        if TCO_ModernHorizontalScrollThumbButton_UpdatePosition then
            TCO_ModernHorizontalScrollThumbButton_UpdatePosition(self)
        end
        if not scroll or scroll.modernVisibleExtent ~= nil then return end
        if scroll.modernScrollSyncing then return end
        if not scroll.SetHorizontalScroll then return end
        scroll.modernScrollSyncing = true
        scroll:SetHorizontalScroll(value)
        scroll.modernScrollSyncing = nil
    end

    function TCO_ModernHorizontalScrollBarTemplate_OnMouseWheel(self, delta)
        TCO_ModernHorizontalScrollFrameTemplate_OnMouseWheel(self:GetParent(), delta)
    end

    function TCO_ModernHorizontalScrollTrackButton_OnLoad(self)
        self:SetFrameLevel(self:GetParent():GetFrameLevel() + 1)
        self:EnableMouseWheel(true)
    end

    function TCO_ModernHorizontalScrollTrackButton_OnMouseDown(self, button)
        if button ~= "LeftButton" then return end
    ...
    function TCO_ModernHorizontalScrollTrackButton_OnMouseWheel(self, delta)
        TCO_ModernHorizontalScrollBarTemplate_OnMouseWheel(self:GetParent(), delta)
    end

    function TCO_ModernHorizontalScrollBarTemplate_OnSizeChanged(self)
        TCO_ModernHorizontalScrollFrameTemplate_Update(self:GetParent())
    end
    ...
    function TCO_ModernHorizontalScrollThumbButton_OnLoad(self)
        local bar = self:GetParent()
        self:SetFrameLevel(bar:GetFrameLevel() + 2)
        ...
        TCO_ModernHorizontalScrollThumbButton_UpdateShape(self)
        TCO_ModernHorizontalScrollThumbButton_UpdatePosition(bar)
        SetModernHorizontalScrollThumbButtonColor(self, c1)
    end

    function TCO_ModernHorizontalScrollThumbButton_OnEnter(self)
        if not self.modernDragging then
            SetModernHorizontalScrollThumbButtonColor(self, c2)
        end
    end

    function TCO_ModernHorizontalScrollThumbButton_OnLeave(self)
        if not self.modernDragging then
            SetModernHorizontalScrollThumbButtonColor(self, c1)
        end
    end

    function TCO_ModernHorizontalScrollThumbButton_OnMouseDown(self, button)
        if button ~= "LeftButton" then return end
    ...
    function TCO_ModernHorizontalScrollThumbButton_OnMouseUp(self, button)
        if button == "LeftButton" and self.modernDragging then
            ModernHorizontalScrollThumbButton_StopDragging(self)
        end
    end

    -- 与 BiaoGe/BGLite 共存时，滚动条更新逻辑复用其全局函数
    if BiaoGe_ModernScrollFrameTemplate_Update then
        TCO_ModernScrollFrameTemplate_Update = BiaoGe_ModernScrollFrameTemplate_Update
        TCO_ModernScrollFrameTemplate_SetScrollExtent = BiaoGe_ModernScrollFrameTemplate_SetScrollExtent
        TCO_ModernScrollFrameTemplate_ClearScrollExtent = BiaoGe_ModernScrollFrameTemplate_ClearScrollExtent
        TCO_ModernHorizontalScrollFrameTemplate_Update = BiaoGe_ModernHorizontalScrollFrameTemplate_Update
        TCO_ModernHorizontalScrollFrameTemplate_SetScrollExtent = BiaoGe_ModernHorizontalScrollFrameTemplate_SetScrollExtent
        TCO_ModernHorizontalScrollFrameTemplate_ClearScrollExtent = BiaoGe_ModernHorizontalScrollFrameTemplate_ClearScrollExtent
    end
end
